...
impl AmountSplitter {
    ...
    fn verify_amount_conservation(&self, original: U256, chunks: &[U256]) -> Result<()> {
        let sum = chunks.iter().fold(U256::zero(), |acc, &chunk| {
            acc.saturating_add(chunk) // Use saturating to prevent overflow
        });
    ...
    fn split_standard(&self, mut amount: U256) -> Result<Vec<U256>> {
        ...
        for &denom in self.standard_denoms.iter().rev() {
            ...
            while amount >= denom && count < max_of_this_denom {
                result.push(denom);
                amount = amount.saturating_sub(denom); // Safe subtraction
                count += 1;
    ...
    fn split_random_safe(&self, amount: U256) -> Result<Vec<U256>> {
        ...
        for i in 0..chunks - 1 {
            ...
            // Random amount up to max_chunk
            let chunk = if max_chunk > U256::from(1) {
                // Generate random bytes and scale to range
                let random_factor = thread_rng().gen_range(1..=100);
                max_chunk.saturating_mul(U256::from(random_factor)) / 100
            } else {
                U256::from(1)
            };

            let chunk = chunk.max(self.min_chunk_size); // Ensure minimum size
            if chunk <= remaining {
                result.push(chunk);
...
impl AmountMixer {
    ...
    pub async fn mix_amounts(&self, entries: Vec<(Address, U256)>) -> Result<Vec<(Address, U256)>> {
        // Calculate original total for verification
