...
use super::merkle_tree::{MerkleTree, TreeState};
...
struct StateSnapshot {
    ...
    tree_root: H256,
    tree_size: usize,
    /// Tree leaves snapshot (full copy for complete restoration)
    tree_leaves: Vec<H256>,
    /// Tree internal nodes snapshot
    tree_nodes: HashMap<(usize, usize), H256>,
    /// Nullifiers before operation
    nullifiers_snapshot: HashSet<H256>,
    /// Pool TVL snapshots
    pool_snapshots: HashMap<H256, U256>,
    /// Pool commitment count snapshots
    pool_commitment_counts: HashMap<H256, usize>,
    /// Pending transitions snapshot
...
impl PrivacyStateManager {
    ...
    fn create_snapshot(&self, tx_id: H256) -> StateSnapshot {
        ...
        StateSnapshot {
            ...
            tree_root: tree.root(),
            tree_size: tree.size(),
            // Export full tree state for complete restoration
            tree_leaves: tree.export().leaves.clone(),
            tree_nodes: tree.export().nodes.clone(),
            nullifiers_snapshot: self.nullifiers.read().clone(),
            pool_snapshots: pools
                .iter()
                .map(|(k, v)| (*k, v.tvl))
                .collect(),
            pool_commitment_counts: pools
                .iter()
                .map(|(k, v)| (*k, v.commitment_count))
                .collect(),
            pending_transitions: self.pending_transitions.read().clone(),
            timestamp: current_timestamp(),
        }
    }

    /// Rollback to previous state snapshot with COMPLETE restoration
    fn rollback_to_snapshot(&self, snapshot: &StateSnapshot) -> Result<()> {
        // CRITICAL FIX: Implement atomic rollback with proper error recovery
        // All state changes must be collected first, then applied atomically

        tracing::info!("Starting atomic rollback for tx_id: {:?}", snapshot.tx_id);

        // Step 1: Prepare all state changes in memory first
        // This ensures we can validate everything before making ANY changes

        // Prepare nullifier rollback
        let new_nullifiers = snapshot.nullifiers_snapshot.clone();

        // Prepare pool state rollback
        let mut new_pool_states = HashMap::new();
        for (pool_id, tvl) in &snapshot.pool_snapshots {
            let count = snapshot.pool_commitment_counts.get(pool_id).copied().unwrap_or(0);
            new_pool_states.insert(*pool_id, (*tvl, count));
        }

        // Prepare merkle tree state - get current height for validation
        let current_height = self.commitment_tree.read().height;

        let tree_state = TreeState {
            height: current_height,  // Height shouldn't change
            leaves: snapshot.tree_leaves.clone(),
            nodes: snapshot.tree_nodes.clone(),
            root: snapshot.tree_root,
        };

        // Validate the new tree state BEFORE applying any changes
        let test_tree = MerkleTree::import(tree_state.clone());
        if test_tree.root() != snapshot.tree_root {
            return Err(anyhow!("Cannot rollback: tree state validation failed"));
        }

        // Prepare pending transitions
        let new_pending = snapshot.pending_transitions.clone();

        // Step 2: Apply ALL changes atomically
        // If ANY operation fails here, we panic to prevent partial state
        // This is safer than partial rollback which leaves inconsistent state

        // Create a rollback guard to ensure atomicity
        ...
        impl Drop for RollbackGuard {
            fn drop(&mut self) {
                if !self.completed {
                    // If we're dropping without completion, something went wrong
                    // Log critical error - state may be inconsistent
                    tracing::error!("CRITICAL: Rollback failed to complete atomically!");
                }
            }
        }

        let mut guard = RollbackGuard { completed: false };

        // Now apply all changes - these should NOT fail if our preparation was correct
        // We use a closure to ensure all locks are acquired in the same order
        let rollback_result = (|| -> Result<()> {
            // Acquire ALL write locks at once to ensure atomicity
            // CRITICAL: Always acquire locks in the same order to prevent deadlock:
            // 1. nullifiers, 2. pools, 3. tree, 4. pending
            ...
            // Apply nullifier changes
            *nullifiers = new_nullifiers;

            // Apply pool changes
            pools.clear();
            for (pool_id, (tvl, count)) in new_pool_states {
                pools.insert(pool_id, PrivacyPool {
                    pool_id,
                    token_address: None,  // Will be set from snapshot if needed
                    tvl,
                    commitment_count: count,
                    params: PoolParams::default(),  // Will be set from snapshot if needed
                });
            }

            // Apply tree changes
            *tree = MerkleTree::import(tree_state);

            // Apply pending transitions
            *pending = new_pending;

            // Verify the rollback succeeded while holding locks
            if tree.root() != snapshot.tree_root {
                // This should never happen if our validation was correct
                return Err(anyhow!("CRITICAL: Tree root mismatch after atomic rollback"));
            }

            if nullifiers.len() != snapshot.nullifiers_snapshot.len() {
                // This should never happen if our preparation was correct
                return Err(anyhow!("CRITICAL: Nullifier count mismatch after atomic rollback"));
            }
        ...
        match rollback_result {
            Ok(()) => {
                ...
                tracing::info!("Atomic rollback completed successfully for tx_id: {:?}", snapshot.tx_id);
                Ok(())
            }
            Err(e) => {
                // Rollback failed - state may be inconsistent
                // This is a critical error that should trigger emergency procedures
                tracing::error!("CRITICAL: Atomic rollback failed: {:?}", e);

                // Try to save current state for debugging
                let current_nullifier_count = self.nullifiers.read().len();
                let current_tree_root = self.commitment_tree.read().root();
                tracing::error!(
                    "State after failed rollback - nullifiers: {}, tree root: {:?}",
                    current_nullifier_count,
                    current_tree_root
                );

                // Return error - the system should handle this as a critical failure
                Err(anyhow!("Atomic rollback failed: {}. State may be inconsistent.", e))
            }
        }
    }
    ...
    pub async fn process_deposit(
        ...
        let result = async {
            ...
            // Amount is now hidden in commitment, verify via range proof
            // The amount verification happens inside ZK proof verification
    ...
    pub async fn process_transfer(
        ...
        let result = async {
            ...
            // Atomic check-and-insert for nullifiers to prevent double-spending
            // CRITICAL: Hold write lock throughout entire check-and-insert operation
            {
                ...
                // Verify merkle root while still holding nullifier lock
                ...
                // Insert nullifiers atomically - no race condition possible
                // since we still hold the write lock from the check above
                for nullifier in &nullifiers {
                    let was_new = nullifier_set.insert(*nullifier);
                    if !was_new {
                        // This should never happen due to our check above,
                        // but handle it as a critical error if it does
                        return Err(anyhow!("CRITICAL: Nullifier race condition detected for: {:?}", nullifier));
                    }
                }
            } // Write lock released here after atomic operation completes
    ...
    pub async fn process_withdrawal(
        ...
        let result = async {
            ...
            // Add nullifier atomically - check and insert in one operation
            {
                let mut nullifiers = self.nullifiers.write();
    ...
    pub fn get_stats(&self) -> PrivacyStats {
        PrivacyStats {
            ...
            total_tvl: self.privacy_pools.read()
                .values()
                .map(|p| p.tvl)
                .try_fold(U256::zero(), |acc, tvl| {
                    acc.checked_add(tvl)
                })
                .unwrap_or(U256::zero()), // Return 0 on overflow (should log error in production)
        }
    }
    ...
    fn test_privacy_state_manager() {
        ...
        let proof = PrivateTransactionProof {
            ...
            public_inputs: PublicInputs {
                ...
                range_proof: vec![],  // Would be actual range proof
            },
