...
impl PrivacyStateManager {
    ...
    fn rollback_to_snapshot(&self, snapshot: &StateSnapshot) -> Result<()> {
        // Use write-ahead log for atomic rollback
        tracing::info!("Starting rollback for tx_id: {:?}", snapshot.tx_id);

        // 1. Rollback nullifiers (atomic operation)
        *self.nullifiers.write() = snapshot.nullifiers_snapshot.clone();

        // 2. Rollback pool states completely
        let mut pools = self.privacy_pools.write();
        for (pool_id, tvl) in &snapshot.pool_snapshots {
            if let Some(pool) = pools.get_mut(pool_id) {
                pool.tvl = *tvl;
                // Restore commitment count
                if let Some(count) = snapshot.pool_commitment_counts.get(pool_id) {
                    pool.commitment_count = *count;
                }
            }
        }

        // Remove any pools created after snapshot
        pools.retain(|k, _| snapshot.pool_snapshots.contains_key(k));

        // 3. COMPLETE Merkle tree restoration
        let mut tree = self.commitment_tree.write();

        // Instead of just rolling back size, restore complete tree state
        ...
        // Import the complete tree state
        *tree = MerkleTree::import(tree_state);

        // 4. Rollback pending transitions
        *self.pending_transitions.write() = snapshot.pending_transitions.clone();

        // 5. Verify rollback succeeded
        if tree.root() != snapshot.tree_root {
            return Err(anyhow!("Tree root mismatch after rollback"));
        }

        if self.nullifiers.read().len() != snapshot.nullifiers_snapshot.len() {
            return Err(anyhow!("Nullifier count mismatch after rollback"));
        }

        tracing::info!("Rollback completed successfully for tx_id: {:?}", snapshot.tx_id);
        Ok(())
    }
    ...
    pub async fn process_withdrawal(
        ...
        let result = async {
            ...
            // Verify amount first (before acquiring write lock)
            if proof.public_inputs.public_amount != amount {
                return Err(anyhow!("Amount mismatch"));
            }
