...
impl PrivacyMerkleTree {
        ...
        if let Some(ref path) = self.nullifier_db_path {
            if let Ok(data) = std::fs::read(path) {
                ...
                for chunk in data.chunks(32) {
                    if chunk.len() == 32 {
                        nullifiers.insert(H256::from_slice(chunk));
                    }
                }
            }
        }
    ...
    pub fn nullify(&mut self, nullifier: H256) -> Result<()> {
        ...
        // Step 2: Atomic check with both sets locked
        {
            let nullifiers = self.nullifiers.lock();
        ...
        // Step 4: Generate transaction ID for atomic operation
        let tx_id = H256::random();
        let wal_entry = WalEntry::AddNullifier { nullifier, tx_id };

        // Step 5: Write to WAL first (atomic operation)
        self.write_to_wal_atomic(&wal_entry)?;

        // Step 6: Persist to disk BEFORE updating memory
        if let Err(e) = self.persist_nullifier_atomic(&nullifier, &tx_id) {
            // Rollback WAL on persistence failure
        ...
        // Step 8: Atomic insert into main set with final check
        {
            let mut nullifiers = self.nullifiers.lock();
            // Final double-check before insert (prevent race during disk operations)
            if nullifiers.contains(&nullifier) {
                let _ = self.rollback_wal(&tx_id);
                return Err(anyhow!("Nullifier race condition detected during finalization"));
            }
            nullifiers.insert(nullifier);
        }

        // Step 9: Mark WAL entry as committed
