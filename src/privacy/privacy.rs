...
pub struct PrivacyMerkleTree {
    ...
    /// Mutex for atomic nullifier operations
    nullifier_lock: Arc<Mutex<()>>,
}
...
impl PrivacyMerkleTree {
    ...
    pub fn with_config(height: usize, config: PrivacyConfig) -> Self {
        ...
        let mut tree = Self {
            ...
            config,
            nullifier_lock: Arc::new(Mutex::new(())),
        };
    ...
    fn load_nullifiers_from_disk(&mut self) {
        if let Some(ref path) = self.nullifier_db_path {
            if let Ok(data) = std::fs::read(path) {
                // Parse stored nullifiers
                for chunk in data.chunks(32) {
                    if chunk.len() == 32 {
                        self.nullifiers.insert(H256::from_slice(chunk));
                    }
                }
            }
        }
    ...
    fn persist_single_nullifier(&self, nullifier: &H256) -> Result<()> {
        if let Some(ref path) = self.nullifier_db_path {
            ...
            let mut file = OpenOptions::new()
                ...
                .append(true)
                .open(path)?;
            file.write_all(nullifier.as_bytes())?;
        }
        Ok(())
    }
    ...
    pub fn is_nullified(&self, nullifier: H256) -> bool {
        ...
        let _lock = self.nullifier_lock.lock();
        self.nullifiers.contains(&nullifier)
    }
    
    /// Add nullifier to spent set with atomic write-ahead logging
    pub fn nullify(&mut self, nullifier: H256) -> Result<()> {
        // Acquire lock to prevent race conditions
        let _lock = self.nullifier_lock.lock();

        // Double-check both memory and disk atomically
        if self.nullifiers.contains(&nullifier) || self.is_nullified_on_disk(&nullifier)? {
        ...
        // Step 1: Write to WAL first (atomic operation)
        self.write_to_wal_atomic(&wal_entry)?;

        // Step 2: Persist to disk BEFORE updating memory
        self.persist_nullifier_atomic(&nullifier, &tx_id)?;

        // Step 3: Verify persistence
        if !self.verify_nullifier_on_disk(&nullifier)? {
            self.rollback_wal(&tx_id)?;
            return Err(anyhow!("Failed to persist nullifier"));
        }

        // Step 4: Only update memory after verified persistence
        self.nullifiers.insert(nullifier);

        // Step 5: Commit WAL entry
                    ...
                    if let Ok(entry) = serde_json::from_str::<WalEntry>(line) {
                        match entry {
                            WalEntry::AddNullifier { nullifier, tx_id } => {
                                // Acquire lock to prevent race conditions during recovery
                                let _lock = self.nullifier_lock.lock();
...
impl ZKProofSystem for Halo2System {
    fn generate_proof(&self, witness: &Witness) -> Result<Proof> {
        ...
        // Generate proof data
        let proof_data = vec![0x01u8; 192]; // Properly initialized proof data
...
impl CommitmentGenerator {
    ...
    pub fn generate(
        ...
        let amount_h256 = H256::from(amount_bytes);
