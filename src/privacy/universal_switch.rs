...
impl PrivacyPool {
    ...
    pub async fn unshield(&mut self, proof: &Proof, nullifier: H256, amount: U256) -> Result<()> {
        ...
        // Atomic check-and-insert using insert's return value
...
impl PrivacyStateManager {
    ...
    pub fn get_pool_readonly(&self, token: H256) -> Option<&PrivacyPool> {
...
impl UniversalSwitch {
    ...
    async fn execute_atomic_switch(
        ...
        // CRITICAL FIX: Establish consistent lock ordering to prevent deadlock
        // Order: 1. mode_balances, 2. pending_switches, 3. privacy_manager, 4. switch_stats
        // This prevents circular dependencies between locks

        // Step 1: Get request info and update balances atomically
        let (needs_shield, commitment, token_id_for_shield, to_mode_for_stats) = {
            // First acquire mode_balances lock
            let mut balances = self.mode_balances.write().await;

            // Then read pending_switches while holding balances (maintains order)
            let pending = self.pending_switches.read().await;
            let request = pending.get(&request_id)
                .ok_or_else(|| anyhow!("Request not found"))?;
            ...
            *to_balance = new_to_balance;
            ...
            // Extract info needed for subsequent operations
            let needs_shield = request.to_mode == TokenMode::Private;
            let commitment = request.commitment;
            let token_id_for_shield = request.token_id.clone();
            let to_mode_for_stats = request.to_mode;

            // Return extracted info and release both locks
            (needs_shield, commitment, token_id_for_shield, to_mode_for_stats)
        };

        // Step 2: Shield if needed (acquire privacy_manager)
        if needs_shield {
            if let Some(commitment) = commitment {
                let mut privacy_mgr = self.privacy_manager.write().await;
                let pool = privacy_mgr.get_pool(token_id_for_shield.0);
                pool.shield(amount, commitment).await?;
            }
        }

        // Step 3: Update statistics (acquire switch_stats)
        {
            ...
            *stats.switches_by_token.entry(token_id_for_shield).or_insert(0) += 1;
            *stats.switches_by_mode.entry(to_mode_for_stats).or_insert(0) += 1;
        }

        // Step 4: Update request status (acquire pending_switches for write)
        {
            let mut pending = self.pending_switches.write().await;
    ...
    pub async fn load_validator_commitments(&self, commitments: Vec<ZKCommitment>) -> Result<()> {
        ...
        for commitment in commitments {
            ...
            if let CommitmentSource::Validator { address, index } = commitment.source {
                ...
                pool.add_validator_metadata(address, index.into(), commitment.block_height).await?;
            }
    ...
    pub async fn mix_and_switch(
        ...
        for (user, amount, secret, nonce) in mixed_entries {
            ...
            let request_id = self.switch_to_private(token_id, user, amount, secret, nonce).await?;
    ...
    pub async fn scan_for_payments(
        ...
        // Simplified implementation - in production would scan blockchain
        Ok(Vec::new())
    }
    ...
    pub async fn verify_switch_for_consensus(
        ...
        let pairs = self.token_pairs.read().await;
        let token_pair = match pairs.get(&request.token_id) {
        ...
        match request.from_mode {
            ...
            TokenMode::Private => {
                ...
                let nullifiers = pool.nullifiers.read().await;
...
mod tests {
    ...
    async fn test_no_deadlock_concurrent_switches() {
        ...
        let switch = Arc::new(UniversalSwitch::new(SwitchConfig::default()));
