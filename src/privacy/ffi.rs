...
use crate::secure_privacy::{SecurePrivacyPool, PrivacyConfig};
use crate::transaction_v2::{TokenId, TokenMode};
use crate::state::GlobalState;
use crate::blockchain::state_connector::StateConnector;
...
pub extern "C" fn register_stealth_keys(
    ...
    // Parse keys
    let spend_key = if spend_key.is_null() {
        ...
        match SecretKey::from_slice(bytes) {
            Ok(key) => key,
            Err(_) => return CString::new("ERROR: Invalid spend key").unwrap().into_raw(),
        }
    };
    ...
    match result {
        Ok(meta_address) => {
            ...
            let spend_pubkey = meta_address.spend_pubkey.serialize();
            let view_pubkey = meta_address.view_pubkey.serialize();
            let response = format!(
