...
/// Returns None if the pointer is invalid or memory is not accessible
unsafe fn validate_handle_wrapper(wrapper_ptr: *const PrivacySystemHandleWrapper) -> Option<&'static PrivacySystemHandleWrapper> {
    ...
    let magic_value = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        Ok(val) => val,
        Err(_) => return None, // Memory not accessible
    };
...
unsafe fn validate_handle_wrapper_mut(wrapper_ptr: *mut PrivacySystemHandleWrapper) -> Option<&'static mut PrivacySystemHandleWrapper> {
...
pub extern "C" fn create_private_transfer(
    ...
    // Validate handle with safe memory access
    let system = unsafe {
        // Check alignment
        ...
        // Safely validate memory region before dereferencing
        // Use volatile read to check if memory is accessible
        let magic_ptr = handle_ptr as *const u64;
        let magic_value = match std::panic::catch_unwind(|| {
            // Try to read the magic value using volatile read
            std::ptr::read_volatile(magic_ptr)
        }) {
            Ok(val) => val,
            Err(_) => return ptr::null_mut(), // Memory not accessible
        };

        // Validate magic number before dereferencing the full struct
        if magic_value != PRIVACY_HANDLE_MAGIC {
            return ptr::null_mut();
        }

        // Now safe to dereference after validation
        let wrapper = &mut *handle_ptr;
        &mut wrapper.handle
    };
...
pub extern "C" fn switch_token_mode(
    ...
    let result = FFI_RUNTIME.block_on(async {
        if to_private == 1 {
            ...
            // Generate actual proof data from transaction parameters
            // Use cryptographic hash to create deterministic proof bytes
            let mut proof_hasher = Sha256::new();
            proof_hasher.update(nullifier.as_bytes());
            proof_hasher.update(owner.as_bytes());
            proof_hasher.update(&amount.to_le_bytes());
            proof_hasher.update(token.0.as_bytes());

            // Generate 192 bytes of proof data
            let mut proof_data = Vec::with_capacity(192);
            for i in 0..6 {
                let mut round_hasher = proof_hasher.clone();
                round_hasher.update(&[i as u8]);
                let hash = round_hasher.finalize();
                proof_data.extend_from_slice(&hash);
            }
            proof_data.truncate(192);
...
pub extern "C" fn switch_private_with_splitting(
    ...
    let secret_h256 = if secret.is_null() {
        ...
        // Validate secret pointer before dereferencing
        if secret.is_null() {
            return ptr::null_mut();
        }
    ...
    let nonce_h256 = if nonce.is_null() {
        ...
        // Validate nonce pointer before dereferencing
        if nonce.is_null() {
            return ptr::null_mut();
        }
...
fn parse_nullifiers(ptr: *const u8, count: usize) -> Vec<H256> {
    // Validate input pointer and count
    if ptr.is_null() || count == 0 {
        return Vec::new();
    }
    // Check for potential overflow in size calculation
    let total_size = match count.checked_mul(32) {
        Some(size) => size,
        None => return Vec::new(), // Overflow detected
    };
...
pub extern "C" fn switch_private_enhanced(
    ...
    // Use safe validation helper
    let wrapper = unsafe {
        match validate_handle_wrapper(handle as *const PrivacySystemHandleWrapper) {
            Some(w) => w,
            None => return CString::new("ERROR: Invalid or corrupted handle").unwrap().into_raw(),
        }
    };
...
pub extern "C" fn register_stealth_keys(
    ...
    // Safe memory validation before dereferencing
    let wrapper = unsafe {
        // Try to read magic value first
        let magic_ptr = wrapper as *const u64;
        let magic_value = match std::panic::catch_unwind(|| {
            std::ptr::read_volatile(magic_ptr)
        }) {
            Ok(val) => val,
            Err(_) => return CString::new("ERROR: Invalid memory access").unwrap().into_raw(),
        };
    ...
    let spend_key_h256 = if spend_key.is_null() {
        ...
        // Already validated spend_key is not null above
    ...
    let view_key_h256 = if view_key.is_null() {
        ...
        // Already validated view_key is not null above
...
pub extern "C" fn scan_stealth_payments(
    ...
    if !ephemeral_pubkeys.is_null() && pubkey_count > 0 {
        // Check for potential overflow in size calculation
        let total_size = match pubkey_count.checked_mul(33) {
            Some(size) => size,
            None => return CString::new("ERROR: Invalid pubkey count (overflow)").unwrap().into_raw(),
        };
        let bytes = unsafe { slice::from_raw_parts(ephemeral_pubkeys, total_size) };
