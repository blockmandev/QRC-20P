        ...
        for (i, sibling) in self.merkle_path.iter().enumerate() {
            current = layouter.assign_region(
                ...
                |mut region| {
                    ...
                    let (left_cell, right_cell) = leaf_bit
                        .map(|bit| {
                            if bit == 0 {
                        ...
                        .unwrap_or((current.clone(), sibling_cell.clone()));

                    // Hash with Poseidon to compute parent node
                    let message = [left_cell, right_cell];
                    let hasher = PoseidonHash::<_, _, P128Pow5T3, ConstantLength<2>, 3, 2>::init(
                        poseidon_chip.clone(),
                        layouter.namespace(|| format!("init poseidon for level {}", i)),
                    )?;

                    hasher.hash(
                        layouter.namespace(|| format!("hash merkle level {}", i)),
                        message,
                    )
                },
            )?;
