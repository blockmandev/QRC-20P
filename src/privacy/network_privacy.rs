...
impl NetworkConnection {
    ...
    fn encrypt_data(&self, data: &[u8], key: &[u8; 32]) -> Result<Vec<u8>> {
        // Implement a secure stream cipher using BLAKE2b in CTR mode
        // This provides actual cryptographic security unlike simple XOR
        ...
        // Create the output buffer with nonce prepended
    ...
    fn decrypt_data(&self, data: &[u8], key: &[u8; 32]) -> Result<Vec<u8>> {
        ...
        while keystream.len() < ciphertext.len() {
            // Create same unique input: key || nonce || counter
            let mut hasher = Params::new()
                .hash_length(64)  // Use full 512-bit output
                .key(key)
                .to_state();
